 ...
 * @classdesc The Plant.Works Web Application Server Alert feature - manages configuration/development for the Alert.
 *
 *
 */
class Alert extends PlantWorksBaseFeature {
	// #region Constructor
	...
	async getConfigDisplayDetails(ctxt) { // eslint-disable-line no-unused-vars
		try {
			...
			mainConfig['attributes']['name'] = 'Alert';
			mainConfig['attributes']['route'] = 'alert';
			mainConfig['attributes']['data_url'] = '/alert/config-tree-nodes';
			...
			let parentFolderId = await dbSrvc.knex.raw(`SELECT id FROM tenant_folders WHERE tenant_id = ? AND name = 'alert_feature.folder_names.root.name'`, [ctxt.state.tenant.tenant_id]);
	 ...
	 *
	 * @summary  Used By the Frontend to display tree menu in IDE for alert
	 */
	async getDevEnvDisplayDetails(ctxt) {
		try {
			...
			mainConfig['attributes']['name'] = 'Alert';
			mainConfig['attributes']['route'] = 'alert';
			mainConfig['attributes']['data_url'] = '/alert/devenv-tree-nodes';
			mainConfig['attributes']['icon_type'] = 'md';
			mainConfig['attributes']['icon_path'] = 'dashboard';
			mainConfig['attributes']['node_type'] = 'root-folder';

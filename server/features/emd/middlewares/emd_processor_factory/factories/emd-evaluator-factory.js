...
/**
 * @class   EmdEvaluatorFactory
 *
 * @param   {object} [options] - Option to pass in the Logger, Cache, and Database interfaces
 *
 * @classdesc The Plant.Works Emd Evaluator Application Class.
 *
 * @description
...
class EmdEvaluatorFactory {
	...
	async getEvaluator(options) {
		try {
			...
			const existingEvaluator = await this._createEmdEvaluatorString(options);
			if(!existingEvaluator) throw new PlantWorksMiddlewareError(`Emd Data Processor cannot be stringified: ${options.id}`);
			...
			if((plantworksEnv === 'development') || (plantworksEnv === 'test')) {
				...
				const generatedCachePath = path.join(rootPath, `generated/emd_${options.id.replace(/-/g, '_')}_evaluator.js`);
			...
			const moduleExports = requireFromString(existingEvaluator, path.join(__dirname, `emd/${options.id}.js`));
			if(!moduleExports.DataProcessor) throw new PlantWorksMiddlewareError(`Emd Data Processor cannot be rendered: ${options.id}`);
			...
			delete require.cache[path.join(__dirname, `emd/${options.id}.js`)];
			return evaluator;
		}
		catch(err) {
			console.error(`Emd Data Processor Instantiation Error:\n`, err.stack);
			throw new PlantWorksMiddlewareError(`Emd Data Processor Instantiation Error`, err);
		}
	}
	 ...
	 * @param    {object} options - Options for creation - mostly used for current timestamp at this point.
	 *
	 * @returns  {undefined} The Emd Data Evaluator as a class.
	 *
	...
	async getEvaluatorString(options) {
		try {
			...
			console.error(`Emd Data Processor Stringification Error:\n`, err.stack);
			throw new PlantWorksMiddlewareError(`Emd Data Processor Stringification Error`, err);
		}
	}
	 ...
	 * @instance
	 * @memberof EmdEvaluatorFactory
	 * @name     _createEmdEvaluatorString
	 *
	 * @param    {object} options - Options for creation - mostly used for current timestamp at this point.
	 *
	 * @returns  {string} An Emd Processor rendered as a Class.
	 *
	 * @summary  Renders the Emd Processor using an EJS Template and returns the rendered string.
	 */
	async _createEmdEvaluatorString(options) {
		try {
			...
			if(options.checkInCache && !options.currentTimestamp && !existingEvaluator && this.$cache && this.$cache.getAsync && (typeof this.$cache.getAsync === 'function')) {
				existingEvaluator = await this.$cache.getAsync(`plantworks!emd!rendered!${options.id}`);
			...
			*/
			...
			if(!existingEvaluator) {
				existingEvaluator = await this._renderEmdEvaluator(options);
			}

			// Step 5: Not in the cache or database, and cannot be rendered - throw up
			if(!existingEvaluator) throw new PlantWorksMiddlewareError(`Emd cannot be rendered: ${options.id}`);
			...
			if(options.setInCache && !inCache && this.$cache && this.$cache.setexAsync && (typeof this.$cache.setexAsync === 'function')) {
				await this.$cache.setexAsync(`plantworks!emd!rendered!${options.id}`, 1800, existingEvaluator);
			}

			return existingEvaluator;
		}
		catch(err) {
			console.error(`_createEmdEvaluatorString: Emd create string error:\n`, err.stack);
			throw new PlantWorksMiddlewareError(`Emd create string error`, err);
		}
	}
	 ...
	 * @instance
	 * @memberof EmdEvaluatorFactory
	 * @name     _renderEmdEvaluator
	 *
	 * @param    {object} options - Options for creation - mostly used for current timestamp at this point.
	 *
	 * @returns  {string} An emd evaluator rendered as a Class.
	 *
	 * @summary  Renders the Machine using an EJS Template and returns the rendered string.
	 */
	async _renderEmdEvaluator(options) {
		try {
			const dbSrvc = this.$database;
			if(!dbSrvc) throw new Error('Database connection unavailable');

			// Step 1: Fetch the emd configuration data
			let emdConfiguration = await dbSrvc.knex.raw(`
SELECT
	id,
	tenant_id,
	...
	operation_metadata AS "operationMetadata"
FROM
	tenant_emd_configurations
WHERE
	id = ?`, [options.id]);

			emdConfiguration = emdConfiguration.rows[0];
			if(!emdConfiguration) throw new Error(`Invalid emdConfiguration id: ${options.id}`);

			if(!emdConfiguration['rawDataTransform'] || (emdConfiguration['rawDataTransform'].trim() === '')) {
				emdConfiguration['rawDataTransform'] = 'return currentData;';
			}

			if(!emdConfiguration['processedDataTransform'] || (emdConfiguration['processedDataTransform'].trim() === '')) {
				emdConfiguration['processedDataTransform'] = 'return evaluatedAttributes;';
			}
			// Step 2: Fetch emdConfiguration attribute data
			let attributeSetIds = await dbSrvc.knex.raw(`SELECT tenant_attribute_set_id FROM tenant_emd_configuration_attribute_sets WHERE tenant_emd_configuration_id = ? ORDER BY evaluation_order ASC`, [emdConfiguration.id]);
			...
			if(attributeSetIds.length) {
				...
				compositeAttributeSet.id = emdConfiguration.id;
				...
				if(typeof emdConfiguration.attributeSetMetadata === 'string') emdConfiguration.attributeSetMetadata = JSON.parse(emdConfiguration.attributeSetMetadata);

				// Step 2.2: Figure out if there is a master data unique parameter, and map it
				emdConfiguration.attributeSetMetadata.forEach((parameterMetadata) => {
					...
					emdConfiguration['uniqueInternalTag'] = uniqueIdParameter ? uniqueIdParameter['internal_tag'] : 'NONE';
				});

				// Step 2.3: Figure out not null paramters
				emdConfiguration['notNullParameters'] = [];
				emdConfiguration.attributeSetMetadata.forEach((parameterMetadata) => {
					if(!parameterMetadata['is_null']) return;
					const nullParameter = compositeAttributeSet['attributes'].filter((compAttribute) => {
						return compAttribute['id'] === parameterMetadata['parameter_id'];
					})[0];

					if(nullParameter)
						emdConfiguration['notNullParameters'].push(nullParameter['internal_tag']);
				});

				emdConfiguration['notNullParameters'] = safeJsonStringify(emdConfiguration['notNullParameters']);


				// Step 2.3: Create the parameterTypeMappings
				compositeAttributeSet.attributes.forEach((emdConfigurationAttribute) => {
					parameterTypeMappings[emdConfigurationAttribute.internal_tag] = emdConfigurationAttribute['data_type'];
					typeMappings[emdConfigurationAttribute.internal_tag] = emdConfigurationAttribute['parameter_type'];
				});
			}

			// Step 3: Get the persistence keyspace
			let keySpace = await dbSrvc.knex.raw(`SELECT sub_domain FROM tenants WHERE id = ?`, [emdConfiguration.tenant_id]);
			...
			const tmplPath = path.join(__dirname, '../templates/emd_data_processor.ejs');
			let renderedEvaluator = await ejs.renderFileAsync(tmplPath, {
				'attributeSetClass': attributeSetClass,
				'emdConfiguration': emdConfiguration,
	...
	async _combineAttributeSets(attributeSetIds) {
		...
		consolidatedAttributeSet['attributes'].forEach((attribute) => {
			...
			if(attribute['data_type'] === 'string') {
				if(attribute['parameter_type'] === 'static')
					attribute['default_value'] = attribute['evaluation_expression'].toString();
				else
					attribute['default_value'] = '';
			}
...
module.exports = function(options) {
	const emdEvaluatorFactory = new EmdEvaluatorFactory(options);
	return emdEvaluatorFactory;
};

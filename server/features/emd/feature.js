 ...
 * @classdesc The Plant.Works Web Application Server External Master Data feature - manages configurtion/development for external master data.
 *
 *
 */
class Emd extends PlantWorksBaseFeature {
	// #region Constructor
	...
	async getDashboardDisplayDetails(ctxt) { // eslint-disable-line no-unused-vars
		try {
			...
			const rbacChecker = this._rbac('emd-read');
			...
			mainDisplay['attributes']['name'] = 'Emd';
			mainDisplay['attributes']['route'] = 'emd';
			mainDisplay['attributes']['icon_type'] = 'mdi';
			mainDisplay['attributes']['icon_path'] = 'database-plus';
	 ...
	 *
	 * @summary  Plant / Unit -> Emd configuration. //DOUBT
	 */
	async getConfigDisplayDetails(ctxt) {
		try {
			...
			mainConfig['attributes']['name'] = 'Emd';
			mainConfig['attributes']['route'] = 'emd';
			mainConfig['attributes']['data_url'] = '/emd/tree-nodes';
			mainConfig['attributes']['icon_type'] = 'mdi';
			mainConfig['attributes']['icon_path'] = 'database-plus';
			mainConfig['attributes']['node_type'] = 'root-folder';

			const dbSrvc = this.$dependencies.DatabaseService;
			let parentFolderId = await dbSrvc.knex.raw(`SELECT id FROM tenant_folders WHERE tenant_id = ? AND name = 'emd_feature.folder_names.root.name'`, [ctxt.state.tenant.tenant_id]);

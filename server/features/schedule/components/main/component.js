 ...
 * @classdesc The Main component of the config Feature - manages CRUD for feature configurations.
 *
 *
 */
class Main extends PlantWorksBaseComponent {
	...
	async _addRoutes() {
		try {
			this.$router.get('/nodes', this.$parent._rbac('schedule-access'), this._getTenantFeatureSchedule.bind(this));
	...
	async _getTenantFeatureSchedule(ctxt) {
		try {
			...
			const configsData = await apiSrvc.execute('Main::getTenantFeatureSchedule', ctxt);
			...
			throw new PlantWorksComponentError(`Error retrieving tenant feature schedule`, err);
		}
	}
	// #endregion

 ...
 * @classdesc The Plant.Works Web Application Server Config Feature Main middleware - manages CRUD for schedule data.
 *
 *
 */
class Main extends PlantWorksBaseMiddleware {
	...
	async _registerApis() {
		try {
			...
			await ApiService.add(`${this.name}::getTenantFeatureSchedule`, this._getTenantFeatureSchedule.bind(this));
	...
	async _deregisterApis() {
		try {
			...
			await ApiService.remove(`${this.name}::getTenantFeatureSchedule`, this._getTenantFeatureSchedule.bind(this));
	...
	// #region API
	async _getTenantFeatureSchedule(ctxt) {
		try {
			...
			for(let idx = 0; idx < serverFeatureNames.length; idx++) {
				...
				const scheduleDisplayDetails = await featureModule.getScheduleDisplayDetails(ctxt);
				if(!scheduleDisplayDetails) continue;

				if(!Array.isArray(scheduleDisplayDetails))
					validFeatures.push(scheduleDisplayDetails);
				else
					scheduleDisplayDetails.forEach((singleDetail) => { validFeatures.push(singleDetail); });
			}
			...
			throw new PlantWorksMiddlewareError(`${this.name}::_getTenantFeatureSchedule`, err);
		}
	}
	// #endregion

...
class Main extends PlantWorksBaseComponent {
	...
	async _addRoutes() {
		try {
			this.$router.get('/nodes', this.$parent._rbac('config-access'), this._getTenantFeatureConfigs.bind(this));
	...
	async _getTenantFeatureConfigs(ctxt) {
		try {
			...
			const configsData = await apiSrvc.execute('Main::getTenantFeatureConfigs', ctxt);
			...
			throw new PlantWorksComponentError(`Error retrieving tenant feature configs`, err);
		}
	}
	// #endregion

...
exports.seed = async function(knex) {
	...
	// Insert the emd feature
	let featureId = await knex.raw(`SELECT id FROM fn_get_module_descendants(?) WHERE name = ? AND type = 'feature'`, [parentId, 'Emd']);
	if(!featureId.rows.length) {
		...
		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'emd-update',
			'implies_permissions': '["emd-read"]'
		});

		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'emd-create',
			'implies_permissions': '["emd-update"]'
		});

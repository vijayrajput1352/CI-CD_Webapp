...
exports.seed = async function(knex) {
	...
	let featureId = await knex.raw(`SELECT id FROM fn_get_module_descendants(?) WHERE name = ? AND type = 'feature'`, [parentId, 'Board']);
	if(!featureId.rows.length) {
		...
		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'board-update',
			'implies_permissions': '["board-read"]'
		});

		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'board-create',
			'implies_permissions': '["board-update"]'
		});

...
exports.seed = async function(knex) {
	...
	let featureId = await knex.raw(`SELECT id FROM fn_get_module_descendants(?) WHERE name = ? AND type = 'feature'`, [parentId, 'EventAlert']);
	if(!featureId.rows.length) {
		...
		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'event-alert-read',
			'implies_permissions': '["config-access", "devenv-access"]'
		});

		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'event-alert-update',
			'implies_permissions': '["event-alert-read"]'
		});

		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'event-alert-create',
			'implies_permissions': '["event-alert-update"]'
		});

		await knex('module_permissions').insert({
			'module_id': featureId,
			'name': 'event-alert-delete',
			'implies_permissions': '["event-alert-update"]'
		});
	}
}

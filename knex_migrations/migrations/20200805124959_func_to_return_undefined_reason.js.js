...
exports.up = async function(knex) {
	await knex.schema.withSchema('public').raw(
		`CREATE OR REPLACE FUNCTION public.fn_ttl_setuptimes (startDate timestamp, endDate timestamp, mId uuid)
		...
		BEGIN
			SELECT SUM((DATE_PART('day', end_time::timestamp - start_time::timestamp) * 24 +
					   ...
					   DATE_PART('minute', end_time::timestamp - start_time::timestamp)) as datediff into ttl from tenant_plant_unit_machine_setuptimes where tenant_plant_unit_machine_id=mId and start_time >= startDate and end_time <= endDate;
			RETURN ttl;
	...
	await knex.schema.withSchema('public').raw(
		`CREATE OR REPLACE FUNCTION public.fn_ttl_setuprzn (startDate timestamp, endDate timestamp, mId uuid)
	...
	await knex.schema.withSchema('public').raw(
		`CREATE OR REPLACE FUNCTION public.fn_ttl_idletimes (startDate timestamp, endDate timestamp, mId uuid)
		RETURNS integer AS $ttl$
		DECLARE
			ttl integer;
		...
		BEGIN
			SELECT SUM((DATE_PART('day', end_time::timestamp - start_time::timestamp) * 24 +
					   DATE_PART('hour', end_time::timestamp - start_time::timestamp)) * 60 +
					   DATE_PART('minute', end_time::timestamp - start_time::timestamp)) as datediff into ttl from tenant_plant_unit_machine_idletimes where tenant_plant_unit_machine_id=mId and start_time >= startDate and end_time <= endDate;
			RETURN ttl;
	...
	await knex.schema.withSchema('public').raw(
		`CREATE OR REPLACE FUNCTION public.fn_ttl_idlerzn (startDate timestamp, endDate timestamp, mId uuid)
		RETURNS integer AS $rzn$
		DECLARE
			rzn integer;
	...
	await knex.schema.withSchema('public').raw(
		`CREATE TABLE IF NOT EXISTS temp_tbl(
				...
				reason_duration_in_min integer,
				reason_code text,
				tenant_plant_unit_machine_setuptime_id uuid,
				tenant_plant_unit_machine_id uuid,
	...
	await knex.schema.withSchema('public').raw(
		`CREATE OR REPLACE FUNCTION public.fn_get_setuptime_rzns (startDate timestamp, endDate timestamp, mId uuid)
	...
	await knex.schema.withSchema('public').raw(
		`CREATE OR REPLACE FUNCTION public.fn_get_idletime_rzns (startDate timestamp, endDate timestamp, mId uuid)
		RETURNS SETOF temp_tbl AS $stoppages$
		DECLARE
			r temp_tbl%rowtype;
			diff integer;
...
exports.down = async function(knex) {
	...
	await knex.schema.withSchema('public').raw('DROP TABLE IF EXISTS public.temp_tbl CASCADE;');
